...
class Metric(object):
    ...
    def _get_thr(self, preds, labels):
        ...
        # min_ind = int(np.argmin(np.array(acers)))
    ...
    def __call__(self, preds, labels, paths=None, thr=None, resfile=None, dataname=None, log_info=True):
        """metric function"""
        ...
        if pos_num != len(labels) and pos_num != 0:
            ...
            for i in range(4):
                ...
                self._thr.append(thrs[int(np.power(10, i))])
        ...
        if resfile is not None:
            ...
            dump(result, os.path.join(self.work_dir, resfile))
